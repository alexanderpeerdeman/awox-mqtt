...
class AwoxConnect:
    ...
    def login(self):
        ...
        response = requests.request(
            "POST", AWOX_CONNECT_URL + 'login', headers=headers, data=payload)
    ...
    def _fetch_class(self, class_name: str):
        ...
        response = requests.request(
            "POST", AWOX_CONNECT_URL + 'classes/' + class_name, headers=headers, data=payload)

        if response.status_code != 200:
            raise Exception('Loading data failed - %s' %
                            response.json()['error'])
